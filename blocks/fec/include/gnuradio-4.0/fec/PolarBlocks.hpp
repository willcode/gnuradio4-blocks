 ...
 *
 * **The CRC is our own.** `ca_scl` asks of each surviving path whether its information bits
 * check, and it is `gr::digital::Crc` under the `crc_*` settings that answers, not AFF3CT's own
 * table — so one polynomial vocabulary serves every block here, and a chain that computes a CRC
 ...
 * log-likelihood ratios in our own sense — positive carries a one. AFF3CT's convention is the
...
struct PolarEncode : Block<PolarEncode> {
    using Description = Doc<"Polar encode: payload records to codeword records under the construction the 'n', 'k' and 'frozen_construction' settings name, with the CRC appended where one is set">;
    ...
    void stop() {
        ...
        detail::report("gr::blocks::fec::PolarEncode", this->name, counters);
    }
...
struct PolarDecode : Block<PolarDecode> {
    using Description = Doc<"Polar decode: soft codeword records to payload records, the channel's account and the CRC verdict accumulating in metadata">;
    ...
    void stop() {
        ...
        detail::report("gr::blocks::fec::PolarDecode", this->name, counters);
    }

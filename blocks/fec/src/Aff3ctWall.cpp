...
PolarCodec::PolarCodec(const PolarSettings& settings) : _impl(std::make_unique<Impl>()) {
    ...
    if (aided && _impl->payload % 8UZ != 0UZ) {
        throw gr::exception(std::format("Polar ({}): the CRC kernel reads whole bytes, so k less crc_width must be a multiple of 8, and it is {}", configuration, _impl->payload));
    }

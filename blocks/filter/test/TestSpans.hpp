#ifndef GNURADIO_FILTER_TEST_SPANS_HPP
#define GNURADIO_FILTER_TEST_SPANS_HPP
...
// Minimal ReaderSpanLike/WriterSpanLike pair, so a processBulk and its forwardTags override can be driven at an exact
// chunk size and from an exact absolute stream position without standing up a graph and a scheduler. The order here is
// the framework's — forwardTags first, then processBulk — because a rate-changing block reads the output span's
// absolute offset to take a new phase origin, and reading it after the samples were written would be a different test.
namespace gr::blocks::filter::test {
...
#endif // GNURADIO_FILTER_TEST_SPANS_HPP

...
class LoopbackDevice : public SoapySDR::Device {
    ...
    std::size_t                                          _overflowEvery   = 0UZ; // 0: never report an overflow
...
public:
    explicit LoopbackDevice(const SoapySDR::Kwargs& args) {
        ...
        if (auto it = args.find("overflow_every"); it != args.end()) {
            auto [ptr, ec] = std::from_chars(it->second.data(), it->second.data() + it->second.size(), _overflowEvery);
            if (ec != std::errc{}) {
                _overflowEvery = 0UZ;
            }
        }
    ...
    int readStream(SoapySDR::Stream* /*stream*/, void* const* buffs, const size_t numElems, int& flags, long long& timeNs, const long timeoutUs = 100000) override {
        ...
        ++_readCalls;
        if (_overflowEvery != 0UZ && _readCalls % _overflowEvery == 0UZ) {

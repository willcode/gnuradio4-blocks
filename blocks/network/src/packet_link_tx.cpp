...
int main(int argc, char** argv) {
    ...
    const auto wireCounters = wire.counters();
    report(text, "packets_sent", wireCounters.packetsSent);
    report(text, "bytes_sent", wireCounters.bytesSent);
    report(text, "rejected_packets", wireCounters.rejectedPackets);
    report(text, "dropped_on_overflow", wireCounters.droppedOnOverflow);
    report(text, "sequence_declined", wireCounters.sequenceDeclined);
    report(text, "meta_keys_mistyped", wireCounters.metaKeysMistyped);
    report(text, "dropped_at_stop", wireCounters.droppedAtStop);
    report(text, "send_errors", wireCounters.sendErrors);

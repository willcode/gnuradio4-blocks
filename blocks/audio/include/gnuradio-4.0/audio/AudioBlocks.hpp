...
private:
    ...
    // ring is far smaller, so the backlog must be clamped to what the port can take:
    // reserving the whole backlog trips the ClaimStrategy's nSlotsToClaim <= size assert in Debug,
    // and the empty-span path discards the entire backlog rather than only the part that does not fit
...
Publishes timing tags with estimated consumption rate and software latency.)"">;
    ...
    // device (hundreds of ms) while Settings::_mutex is held; record the request and let the I/O
    // thread apply it between spans instead

    ...
    [[nodiscard]] std::expected<bool, gr::Error> tryParseHeader() {
        ...
        while (pos + 8U <= bytes.size()) {
            ...
            if (detail::bytesEqual(chunkId, "data")) {
                ...
                // a recording cut short by a crash declares 0 (or a stale over-long size), so the
                // physical file length is trusted over the header
...
private:
    // the cap counts the header, and a freshly opened file takes at least one sample, so a cap too
    // small for the header plus one sample still makes progress

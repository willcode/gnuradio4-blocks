...
const boost::ut::suite<"DataSetToPacket"> dataSetToPacketTests = [] {
    ...
    // criterion 9, the receiver's own fact — a chain is handed rates and gains and never a tuning, so `frequency` is
    ...
    // encoding, and its emitter wrote `std::ostream`'s default six significant digits until the fork's fix: a tuning
    // near 1 GHz came back rounded to 10 kHz, and a receiver on a channel center saw no error at all, which is what
    // made the loss hard to notice. Round numbers are therefore not enough to assert on, and none of these is one.

...
const boost::ut::suite<"ChunkReassembler"> chunkReassemblerTests = [] {
    ...
    // Out-of-order reassembles identically to in-order
    ...
    // Duplicates with equal content are counted and change nothing
    ...
    // A conflicting duplicate does not overwrite
    ...
    // A missing chunk never completes and is evicted with a count
    ...
    // Two interleaved files complete independently
    ...
    // Max_open_files evicts the least recently touched and admits the new file
    ...
    // Max_file_bytes refuses by range, not by allocation, and the guard is checked before the multiply
    ...
    // Max_gaps refuses only chunks that would add a gap (the interval path, via 'offset')
    ...
    // Completion is coverage, not a write pointer
    ...
    // The two declarations, and the conflict rule
    ...
    // Chunk independence and CRC accounting
